...
6. Write audit logs for new and unknown-wallet deposits by default; use the listener's verbose audit policy when duplicate and ignored event audit records are required for backfill or incident review.
...
The state-changing listener unit coverage separately confirms that duplicate and ignored events are silent under the default `state_changes_only` audit policy, while `auditPolicy: "verbose"` emits `deposit.duplicate_skipped` and `deposit.ignored` audit entries. It also confirms that explicit scanned checkpoints advance for no-log and all-ignored ranges.
